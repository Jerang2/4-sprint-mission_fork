...
// 내 정보 조회 API
router.get('/me', auth_middleware_1.default, async (req, res, next) => {
    try {
        const { user } = req;
        if (!user) {
...
// 내 정보 수정 API
router.patch('/me', auth_middleware_1.default, async (req, res, next) => {
    try {
...
// 비밀번호 변경 API
router.patch('/me/password', auth_middleware_1.default, async (req, res, next) => {
    try {
...
// 내가 작성한 상품 목록 조회 API
router.get('/me/products', auth_middleware_1.default, async (req, res, next) => {
    try {
        const { user } = req;
        if (!user) {
            return res.status(401).json({ message: '사용자 정보를 찾을 수 없습니다.' });
